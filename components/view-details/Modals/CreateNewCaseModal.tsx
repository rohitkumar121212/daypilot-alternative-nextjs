...
import CreateCaseTab from '@/components/ReservationChart/Modals/BookingDetailsModal/CreateCaseTab'

interface CreateNewCaseModalProps {
  isOpen: boolean
  onClose: () => void
  bookingDetails: any
}

const CreateNewCaseModal = ({ isOpen, onClose, bookingDetails }: CreateNewCaseModalProps) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([{ data: assignToUsersData }, { data: reservationConstantsData }]) => {
        const modified = assignToUsersData?.data?.map((user: any) => ({
          value: user.id,
          label: `${user.name} (${user.email})`,
        }))
        setAssignToUsers(modified || [])
        setReservationConstants(reservationConstantsData?.data)
      })
      .catch((err) => console.error('Failed to fetch case modal data:', err))
  }, [isOpen])
    ...
    >
      ...
      >
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <h2 className="text-xl font-semibold text-slate-800">Create New Case</h2>
