...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch('https://aperfectstay.ai/api/pms-enqire-split-booking', {
      method: 'POST',
      headers,
    ...
    console.log('proxy split-booking Booking created successfully:', data)
    ...
    return NextResponse.json({ error: 'Failed to split reservation', details: String(error) }, { status: 500 })
  }
}
